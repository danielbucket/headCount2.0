...
describe('DistrictRepository iteration 1 - part 1', () =>  {
  ...
  test('1. findByName returns undefined if no arguments are provided', () => {
    expect(district.findByName()).toBe(undefined);
  });

  test('2. findByName returns undefined when there are no matching arguments', () => {
    expect(district.findByName('Vikings')).toBe(undefined);
  });

  test('3. findByName returns an object with its individual district information', () => {

    expect(typeof district.findByName('Colorado')).toEqual('object');
    expect(district.findByName('Colorado').location).toEqual('COLORADO');
  });

  test('4. findByName search is not case sensitive', () => {
    expect(district.findByName('ColoRAdo').location).toEqual('COLORADO');
    expect(district.findByName('ACADEmY 20').location).toEqual('ACADEMY 20');
  });

  test('5. each district has a data object containing each year and its data as key value pairs.', () => {
  ...
  test('6. district data is rounded to the nearest hundredth', () => {
  ...
  test('7. district data is sanitized and defaults to 0', () => {
